...
export async function saveGarageCustomerToCloud(customer:any){
 ...
 const legacyId=String(customer.id),rows=await supabaseUserRequest<{id:string}[]>(`customers?organization_id=eq.${organizationId}&legacy_source_id=eq.${encodeURIComponent(legacyId)}&select=id`,token),body={organization_id:organizationId,legacy_source_id:legacyId,customer_type:customer.company?'business':'personal',name:customer.name,company_name:customer.company||null,email:customer.email==='Not recorded'?null:customer.email||null,phone:customer.phone||null,address:customer.address==='Not recorded'?null:customer.address||null,created_by:actorId||null}
...
export async function saveGarageVehicleToCloud(ownerLegacyId:string,vehicle:any){
 const{token,organizationId}=context();if(!token||!organizationId)return
 const owners=await supabaseUserRequest<{id:string}[]>(`customers?organization_id=eq.${organizationId}&legacy_source_id=eq.${encodeURIComponent(ownerLegacyId)}&select=id`,token);if(!owners[0])throw new Error('Save the customer to the cloud before adding their vehicle.')
