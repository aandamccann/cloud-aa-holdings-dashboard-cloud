...
let started=false
export function startGarageEstimatesCloudSync(){if(started)return;started=true;const nativeSet=Storage.prototype.setItem;let timer:number|undefined,pulling=false;const report=(error:unknown)=>window.dispatchEvent(new CustomEvent('aa-cloud-sync-error',{detail:error instanceof Error?error.message:'Estimate cloud sync failed.'})),refresh=async()=>{if(pulling||document.visibilityState==='hidden')return;pulling=true;try{await hydrateGarageEstimatesFromCloud()}catch(error){report(error)}finally{pulling=false}};Storage.prototype.setItem=function(key:string,value:string){nativeSet.call(this,key,value);if(this===localStorage&&key.startsWith('aa-garage-quotes-')){clearTimeout(timer);timer=window.setTimeout(()=>void syncLocalGarageEstimatesToCloud().catch(report),200)}};window.setInterval(()=>void refresh(),15000);window.addEventListener('focus',()=>void refresh());document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')void refresh()})}
