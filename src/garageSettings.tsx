...
import { Settings, ShieldAlert, Save, Search, Wrench, PackagePlus, Trash2, FileText } from 'lucide-react';
...
export const readGarageRates = (): GarageRates => {
  try {
    ...
    return { labourRate: 100, consumablesDefault: 12, wasteDefault: 0, quoteCustomerNote: '', partsVatRate:23, labourVatRate:13.5, consumablesVatRate:13.5, vatAccountingBasis:'unconfirmed', ...stored, vatRate, vatRates };
  } catch {
    return { vatRate: 13.5, vatRates: [13.5, 23], partsVatRate:23, labourVatRate:13.5, consumablesVatRate:13.5, vatAccountingBasis:'unconfirmed', labourRate: 100, consumablesDefault: 12, wasteDefault: 0, quoteCustomerNote: '' };
  }
};
...
export function GarageSettings({initialTab='rates'}:{initialTab?:'rates'|'custom-jobs'|'parts'|'vat-reports'}) {
  const mainAccount = (sessionStorage.getItem('aa-role') || 'owner') === 'owner';
  const [rates, setRates] = useState(readGarageRates);
  const [activeTab, setActiveTab] = useState<'rates' | 'custom-jobs' | 'parts' | 'vat-reports'>(initialTab);
  ...
  useEffect(() => { const heading = document.querySelector('main>header h1'); if (heading) heading.textContent = initialTab==='vat-reports'?'Garage reports':'Garage settings'; }, [initialTab]);
  ...
  return <div className="garageSettingsPage">
    ...
    <nav className="garageSettingsTabs" aria-label="Garage settings sections">
      ...
      <button className={activeTab === 'vat-reports' ? 'active' : ''} type="button" onClick={() => setActiveTab('vat-reports')}><FileText/>VAT reports</button>
    </nav>
    {activeTab === 'rates' && <section>
      ...
      <div className="garageSettingsGrid">
        ...
        <label><span>Default VAT on parts %</span><input disabled={!mainAccount} type="number" min="0" step="0.1" value={rates.partsVatRate} onChange={event=>change('partsVatRate',+event.target.value)}/></label>
        <label><span>Default VAT on labour %</span><input disabled={!mainAccount} type="number" min="0" step="0.1" value={rates.labourVatRate} onChange={event=>change('labourVatRate',+event.target.value)}/></label>
        <label><span>Default VAT on Consumables &amp; Waste %</span><input disabled={!mainAccount} type="number" min="0" step="0.1" value={rates.consumablesVatRate} onChange={event=>change('consumablesVatRate',+event.target.value)}/></label>
        ...
        <label className="quoteCustomerNoteSetting"><span>Default customer note on every Estimate</span><textarea disabled={!mainAccount} value={rates.quoteCustomerNote} onChange={event => setRates(current => ({ ...current, quoteCustomerNote: event.target.value }))} placeholder="Enter the standard instructions or notes shown at the bottom of every customer Estimate"/></label>
      </div>
