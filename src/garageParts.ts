export type GaragePart={id:string;name:string;description:string;partNumber?:string;sku:string;unit:string;price:number;cost:number;stock:number;supplier:string;vatRate:number};
...
export const defaultGarageParts:GaragePart[]=[
 {id:'part-oil-filter',name:'Oil filter',description:'Standard engine oil filter',sku:'OF-001',unit:'each',price:12.5,cost:0,stock:0,supplier:'',vatRate:23},
 {id:'part-air-filter',name:'Air filter',description:'Engine air filter',sku:'AF-001',unit:'each',price:18,cost:0,stock:0,supplier:'',vatRate:23},
 {id:'part-fuel-filter',name:'Fuel filter',description:'Fuel system filter',sku:'FF-001',unit:'each',price:28,cost:0,stock:0,supplier:'',vatRate:23},
 {id:'part-engine-oil',name:'Engine oil 5W-30',description:'Fully synthetic engine oil',sku:'OIL-5W30',unit:'litre',price:11.5,cost:0,stock:0,supplier:'',vatRate:23},
 {id:'part-front-pads',name:'Front brake pad set',description:'Front axle brake pads',sku:'BP-FRONT',unit:'set',price:64,cost:0,stock:0,supplier:'',vatRate:23},
];

export const readGarageParts=():GaragePart[]=>{try{const saved=JSON.parse(localStorage.getItem(garagePartsKey)||'null');return Array.isArray(saved)?saved.map(part=>({...part,vatRate:Number(part.vatRate??23)})):defaultGarageParts}catch{return defaultGarageParts}};
